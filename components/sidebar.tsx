...
import {
  ...
  useSpring, useTransform, MotionConfig,
} from 'motion/react'
...
// Item individual com magnetic hover e spring physics
function NavLink({ href, label, icon: Icon, color, index }: {
  href: string; label: string; icon: React.ElementType; color: string; index: number
}) {
...
// Seção colapsável com spring
function NavSection({ section, sectionIndex }: { section: NavSection; sectionIndex: number }) {
  const pathname = usePathname()
          ...
          >
            ...
            >
              {section.items.map((item, i) => (
                <NavLink key={item.href} {...item} color={section.color} index={i} />
              ))}
            </motion.div>
          </motion.div>
        )}
      ...
      >
          ...
          >
            ...
            <div>
              ...
              <motion.p
                ...
                style={{ color: 'rgba(255,255,255,0.28)', fontSize: 9, letterSpacing: 2.5, textTransform: 'uppercase', margin: '2px 0 0' }}
              >
                Alimentos · Gestão
              </motion.p>
            </div>
        ...
        <nav style={{ flex: 1, padding: '4px 8px', overflowY: 'auto' }}>
          {sections.map((section, i) => (
            <NavSection key={section.id} section={section} sectionIndex={i} />
          ))}
        </nav>
